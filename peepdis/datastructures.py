...
class Arg(_PreferencesMixin):
    def __init__(self, name, value: Any = None, type_: Type = None,
                 method: str = None):
        self.name = name
        self.value = value
        self.type_ = type_
...
class ArgDict(OrderedDict, _PreferencesMixin):
    ...
    @property
    def null_args(self):
        nulls = tuple([k for k, v in self.items()
                       if v.value is None])
        return nulls
    ...
    def __setitem__(self, key, value):
        if not isinstance(value, Arg):
            raise TypeError(
                f'`ArgDict` values must be type `Arg`, not: {type(value)}')
        if isinstance(key, int):
            key = list(self.keys())[key]
...
class Output(_PreferencesMixin):
    ...
    _dunders = {'__dict__', '__module__', '__weakref__',
                '__slotnames__'}
    _builtins = {x for type_ in _types for x in dir(type_)} | _dunders

    def __init__(self, name: str, value: Any, args: ArgDict = None,
                 callable_: bool = False, error: bool = False,
